...
class NewRegistratoin extends React.Component {
  ...
  handleRegistration = async (event) => {
    ...
    const { email, password, username, full_name, role, skill, phone_number, school_name, major, affiliations, i_agree_to_the_rules_of_the_competition } = this.state
    const data = { email, password, username, full_name, role, skill, phone_number, school_name, major, i_agree_to_the_rules_of_the_competition }
    var validation = `${
      !role ? 'Please select your role' :
        !full_name ? 'Please enter your Full Name' : !username ? 'Please enter your User Name' : !email ? 'Please enter your Email' : !emailRegex.test(email) ? 'Please enter valid Email' :
          !password ? 'Please enter Password' : password.length < 8 ? 'Enter Password must be above 8 characters' :
            !phone_number ? 'Please enter Phone Number' :
              !school_name && role !== 'judge' ? 'Please enter School Name' :
                !major && role !== 'judge' ? 'Please enter Major Name' :
                  !affiliations && role !== 'judge' ? 'Please select Affiliation with UCI' :
                    true}`
    if (validation === 'true') {
      ...
      await fetch(routes.baseURL + 'user/signup/', {
        ...
        if (responseBody.status === 200 && responseBody.msg) {
          if (self.state.role === 'judge') {
            ...
            session.setSession(responseBody.data.token, responseBody.data.token, JSON.stringify(responseBody.data));
  ...
  handleCheckBox = () => {
    this.setState({
      i_agree_to_the_rules_of_the_competition: !this.state.i_agree_to_the_rules_of_the_competition
    })
  }
  render() {
    const options = [
            ...
            <div className="auth-modal">
              ...
              <form onSubmit={this.handleRegistration}>
                ...
                <input
                  ...
                  required
                />
                {this.state.role === 'participant' ? <div><input
                  type="text"
                  name="school_name"
                ...
                />
                  ...
                  <Select className="fadeInAnimation myClassName" onChange={this.affiliationsHandler} options={affiliationOptions} />
                </div> : ""}
                ...
                <div class="form-group custom-checkbox">
                  <input type="checkbox" id="select0" name="feedback" onChange={(e) => this.handleCheckBox()} checked={this.state.i_agree_to_the_rules_of_the_competition} />
                  <label class="bg-black text-white" for="select0">
                    I Agree to the Privacy Policy of Kontess and Rules of the Competition
                  </label>
                </div>
                ...
                {this.state.loading ? <div className="loadingContainer"><div className="ui active inline loader"></div> </div> :
                  <button type="button" className="button" onClick={this.handleRegistration} disabled={!this.state.i_agree_to_the_rules_of_the_competition}>
