...
export default class MyTeam extends React.Component {
  ...
  getAllUsers = async (type, offset = 0, search = 0) => {
    ...
    await getFetch(`user/?role=${type}&limit=1000&chat-list=true&offset=${offset}&name=${search ? search : ''}`).then((resp) => {
      self.setState({
        loading: false
      })
      if (resp.data) {
  ...
  getMyTeams = async (offset = 0) => {
    let self = this;
    await getFetch('team/myteam/?limit=1000&offset=0').then((responseBody) => {
            ...
            <div className="row">
                  ...
                  <div className="chat-message p-3">
                    <div className="input-group mb-3">
                      <div className="input-group-prepend">
                        {this.state.chatWith ? <label className="input-group-text border-0" htmlFor="choose-logo">
                      ...
                      {this.state.chatWith ? <button className=" input-group-append input-group-text border-0 text-primary" onClick={this.handleSend.bind(this)}><i class="fas fa-paper-plane fa-lg mr-2"></i> Send</button> : ''}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="col-md-3">
                <div className="people-list bg-white py-3 px-4">
                  {this.state.userType !== 'admin' ?
                    <div className="list-group">
                      ...
                      <ul className="dflex mb-2 justify-content-center list chatUsersList flex-column">
                        {myTeamList.data ? myTeamList.data.length ? myTeamList.data.map(team => (
                        ...
                        )) : <h6 className="text-center">You currently are not in any team.</h6> : <Loading />}
                      </ul>
                    </div> : ''}
                  {this.state.userType !== 'admin' ?
                    <div className="list-group">
                  ...
                  <div className="list-group">
                    <div className="clearfix border-bottom pb-2 mb-3">
                      <div className="float-left"><h5 className="chat-user-list mb-0 border-bottom-0">Judges/Coachs</h5></div>
                    </div>
                    <ul className="dflex mb-2 justify-content-center list chatUsersList flex-column">
