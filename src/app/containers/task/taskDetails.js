...
export default class TaskDetails extends React.Component {
    ...
    getAllTeams = async (offset = 0, track = '') => {
        let self = this;
        self.setState({
            dataList: [],
            curruntTrack: track
        })
        await getFetch('track/' + track + '/team-list/?task_id=' + this.state.taskId + '&limit=' + this.state.perPage + '&offset=' + offset).then((responseBody) => {
    ...
    getAllUsers = async (type, offset = 0, search = 0, skill = '') => {
        ...
        await getFetch(`user/?role=${type}&limit=${500}&offset=${offset}&name=${search ? search : ''}&skill=${skill}`).then((resp) => {
            if (resp.data) {
                data[type] = resp.data;
    ...
    getTaskStatus = (submissionDueDate, gradeDueDate) => {
        ...
        if (moment() < submission_due_date) {
            ...
            result = <div> <h6 className={`d-block pt-2 m-0 ${text_class}`}>{status}</h6><hr /> <button title="You can release score after grading completed" className="btn btn-md btn-primary px-4 rounded-0" disabled>Relase Score to Participants</button></div>
            ...
            result = <div><h6 className={`d-block pt-2 m-0 ${text_class}`}>{status}</h6> <hr />
                {this.state.UserType === 'admin' ? this.state.release_score_to_participant ? <span className="badge badge-success">Score Relased to Participants</span> :
                    <button onClick={() => this.relaseScore()} className="btn btn-md btn-primary px-4 rounded-0">Relase Score to Participants</button>
                    : ''}
            </div>
        }
        return result
    ...
    render() {
        const { title, judge, description, submission_due_date, grade_due_date, questions, max_no_of_judge, assing_to } = this.state;
        return (
                                    ...
                                    <div id="taskDetails" className="collapse" data-parent="#accordion">
                                        <div className="card-body">
                                            <div className="grading-wrapper">
                                                ...
                                                <h6>Maximum number of judges: {max_no_of_judge}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="col-md-3 juges">
                                        <div className="card border border-secondary mb-3">
                                            <div className="card-head py-2 px-3 border-707070">
                                                <h6 className="m-0 text-333f52">Judges assigned</h6>
                                            </div>
                                            <div className="card-body px-3 py-0 list">
                                                ...
                                                )) : <h5 className="text-center mt-5" >No judge assigned to this track!</h5> : <Loading />}
                                            </div>
                                        </div>

                                        <div className="card border border-secondary">
                                            <div className="card-head py-2 px-3 border-707070">
                                                <h6 className="m-0 text-333f52">List of all judges</h6>
                                            </div>
                                            <div className="card-body px-3 py-0 list">
