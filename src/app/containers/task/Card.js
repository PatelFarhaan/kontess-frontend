  ...
  showOverAll = (task_grading, id) => {
    if (this.state.curruntUserType === 'admin') {
      ...
      if (task_grading.length) {
        ...
        return show ? <div> <h5 className="text-danger">Overall : {result}</h5><a className="text-primary  pt-1 d-block font-weight-bold" data-toggle="collapse" data-target={"#collapseOne" + id} aria-expanded="false" aria-controls="collapseOne">View Details</a></div> : '';
      }
    }
  }
        ...
        {this.props.type === 'teams' ? this.props.teamList ? this.props.teamList.length ? this.props.teamList.map(team => (
          <div key={team.name} className="card my-2">
            ...
            {curruntUserType === 'admin' ? <div id={"collapseOne" + team.id} className="collapse" aria-labelledby="headingOne" data-parent="#accordionExample" >
              <div className="col-md-12 pb-3"><ResultView judges={this.props.judges} questions={this.props.questions} task_grading={team.task_grading}></ResultView></div>
            </div> : ''}
          </div>
