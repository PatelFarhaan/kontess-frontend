  ...
  getGradingProgress = (task_grading_counts, due) => {
    if (task_grading_counts) {
      ...
      return <div>
        ...
        <small> {task_grading_counts.judge_graded} out of {task_grading_counts.total_no_of_juges} judges finished grading </small>
      </div>
    }
  }
              ...
              {data.data ? data.data.length ? data.data.map(item => (
                <div className="card mb-1 mt-mob-4">
                  <div className="card-body p-2">
                    <strong className="font-family-open">{item.title}</strong>
