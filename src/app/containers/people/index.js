...
export default class People extends React.Component {
    ...
    getAllUsers = async (type, offset = 0, search = 0, skill = '') => {
        ...
        await getFetch(`user/?role=${type}&limit=${self.state.perPage}&offset=${offset}&name=${search ? search : ''}&skill=${skill}`).then((resp) => {
            self.setState({
                loading: false
            })
            if (resp.data) {
                            ...
                            {this.state.UserType !== 'admin' ?
                                <div>
                                    ...
                                    <div className="ui pointing secondary menu">
                                        ...
                                        <a className={`item ${this.state.type === 'judge' ? 'active' : ''}`} onClick={(e) => this.getAllUsers('judge')}>Judges</a>
                                    </div>
