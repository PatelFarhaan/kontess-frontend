import React from 'react';
import './style.scss';

const facebookIcon = 'data:image/svg+xml;base64,';
const twitterIcon = 'data:image/svg+xml;base64,';
const instagramIcon = 'data:image/svg+xml;base64,';

const Footer = () => (
  <footer>
    <span>
      <p>© 2019 Kontess</p>
    </span>
    <span>
      <a href={'https://www.facebook.com/Jiffal/?view_public_for=402512323491534'} target={'_blank'} rel={'noopener noreferrer'}><img src={facebookIcon} alt={'facebook'} /></a>
      <a href={'https://twitter.com/jiffal'} target={'_blank'} rel={'noopener noreferrer'}><img src={twitterIcon} alt={'twitter'} /></a>
      <a href={'https://www.instagram.com/jiffal/'} target={'_blank'} rel={'noopener noreferrer'}><img src={instagramIcon} alt={'instagram'} /></a>
    </span>
    <a href={'https://docs.google.com/forms/d/e/1FAIpQLScJfW3kMg933fBCWOh4rvITq20m8xWSsnHpoOtagNnS86b-gA/viewform'} target={'_blank'} rel={'noopener noreferrer'}>
      Help us improve Kontess
    </a>
  </footer>
);

export default Footer;
